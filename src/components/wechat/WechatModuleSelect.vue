...
<template>
  <div ref="rootRef" class="relative w-full min-w-0">
    <button
      ...
      @click.stop="toggle"
    >
      <span class="min-w-0 truncate text-muted">插入模块…</span>
      <ChevronDown class="h-3 w-3 shrink-0 text-muted" aria-hidden="true" />
    </button>

    <div
      v-if="open"
      class="scrollbar-thin absolute left-0 top-full z-50 mt-1 w-full min-w-[180px] max-h-[280px] overflow-y-auto rounded-lg border border-border bg-surface-1 p-1"
